...
if [[ ! -f upoints ]]; then
        echo "copy (select st_y(utilisateur.the_geom) as y, st_x(utilisateur.the_geom) as x from utilisateur,espace_departement where st_intersects(utilisateur.the_geom,espace_departement.the_geom) and utilisateur.the_geom is not null and espace_departement.reference in ('02','60','80') and utilisateur.id_utilisateur in (select oo.id_utilisateur from observations o, observations_observateurs oo WHERE o.id_observation=oo.id_observation and extract('year' from date_observation) =  2017)) to stdout;"|psql -h sgc clicnat pn > upoints
fi
