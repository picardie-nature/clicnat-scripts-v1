...
$sql = "
	SELECT 	ecom.nom, etag.v_text AS \"Type\", 
		ec.reference, e.nom_f,
		min(c.nb) AS \"EffectifMini\",
		max(c.nb) AS \"EffectifMaxi\",
		min(date_part('year', o.date_observation)) AS \"PremiereObs\",
		max(date_part('year', o.date_observation)) AS \"DerniereObs\",
		astext(ec.the_geom) AS geom 
	FROM 
		observations o,
 		citations c,
		espace_chiro ec,
		...
		espace_commune ecom,
		espace_tags etag 
	WHERE o.espace_table='espace_chiro' 
	AND not brouillard
	AND coalesce(c.nb,0) != -1
	AND ec.id_espace=o.id_espace 
	AND c.id_observation=o.id_observation 
	AND c.id_espece=e.id_espece
	AND ec.commune_id_espace=ecom.id_espace 
	AND etag.id_tag=413
	AND etag.id_espace=ec.id_espace
	AND etag.espace_table='espace_chiro'
	GROUP BY ecom.nom, etag.v_text, ec.reference, e.nom_f, ec.the_geom
	ORDER BY ecom.nom, ec.reference, e.nom_f";
